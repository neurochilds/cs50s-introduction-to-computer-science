...
bool check(char *word)
{
    node *cursor = root; // Point cursor to root of tree
    for (int i = 0, len = strlen(word); i < len; i++) // Loop over each character in the name
    {
        int index = tolower(word[i]) - 'a'; // Convert ASCII character to integer index
        if (cursor->children[index] == NULL)
        {
            return false; // If nothing exists at index, return false
        }
        cursor = cursor->children[index];
    }
    return cursor->is_word; // Once we've travelled down the trie for each character in the name, check if the name exists in the trie
}
